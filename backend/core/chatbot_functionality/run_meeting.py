...
def clear_local_s3(username):
    if (settings.USE_S3):
        #clear the local knowledge bases
        path = f'{username}_knowledge_bases'
...
def run_meeting(params):
    ...
    try:
        ...
        for agent in agents_data:
            ...
            agents_map[agent_name] = Agent(agent_role, expertise, kb_path=agent_kb_path)
        ...
        **Important:** 
        ...
        print("decided agents")
        #for each entry in worker_team, set their kbs(given each agent has attr kb_path)
        print(worker_team)
        for agentname, agent_obj in worker_team.items():
            if (settings.USE_S3):
                #saves the s3 kb into local
                print("THE PATH THAT IS BEING SAVED:", agent_obj.kb_path)
                s3.open_s3(f'{username}_knowledge_bases/agent/{agentname}', agent_obj.kb_path)
                #saves the kb into lead_path
            AgentKB = KnowledgeBase.from_path(agent_obj.kb_path)
            worker_team[agentname].set_memory(kb=AgentKB)
