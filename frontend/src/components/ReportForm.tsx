...
interface ReportFormProps {
    ...
    error: string | null;
    onErrorChange: (newError: string | null) => void;
...
function ReportForm({name, task, lead, onErrorChange, onSelectFileChange, onRemoveFile, onSelectAgentChange, expectations, description,reportGuidelines, uploadedFiles, cycles, temperature, onFilesChange,onNameChange, onTaskChange, onLeadChange,onCyclesChange, onExpectationsChange, onEngineChange, onReportGuidelinesChange, onModelChange, onDescriptionChange, onTemperatureChange,onSubmit}: ReportFormProps) {
    const steps = ['Name', 'Task details', 'Team selection', 'Context', 'Conversation settings']
    ...
    const handleNext = () => {
        ...
        if (activeStep === 0 && (!name)) {
            onErrorChange("Name required");
            return;
        }
        if (activeStep === 1 && (!task)) {
            onErrorChange("Task required");
            return;
        }
        if (activeStep === 2 && (!lead)) {
            onErrorChange("Lead required");
            return;
        }
        ...
        <Box>
            <Stepper activeStep = {activeStep} sx={{ mb: 4}}>
                ...
                <Box display="flex" justifyContent="space-between" mt={2}>
                    <Button onClick={handleBack} disabled={activeStep === 0} variant="outlined" type="button">Back</Button>
                    <Button onClick={handleNext} variant="contained" type="button">Next</Button>
                </Box>
                )}

                {activeStep === steps.length - 1 && (
                <Box component="form" onSubmit={onSubmit} display="flex" justifyContent="space-between" mt={2}>
